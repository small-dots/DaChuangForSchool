<template>
  <a-row class="user-select" :gutter="16">
    ...
    <a-col :span="6">
      <selected-list
        v-model:list="state.approverList"
        @delete="deleteApprover"
        @deleteAll="deleteAll"
      ></selected-list>
    </a-col>
  </a-row>
...
<script setup lang="ts">
  ...
  const onSelect = (record: any, selected: boolean, selectedRows: any) => {
    ...
    if (selected) {
      ...
      if (
        !state.approverList.find(
          (userItem) => userItem.bizId == record.code && userItem.subValue == state.orgId,
        )
      ) {
      ...
      state.approverList.splice(
        state.approverList.findIndex(
          (item) => item.bizId === record.code && item.subValue == state.orgId,
        ),
        1,
      );
    }
  ...
  const onSelectAll = (selected: boolean, selectedRows: any, changeRows: any) => {
    if (selected) {
      ...
      changeRows.forEach((item) => {
        if (
          !state.approverList.find(
            (userItem) => userItem.bizId == item.code && userItem.subValue == state.orgId,
          )
        ) {
          let obj = {
            bizId: item.code,
      ...
      changeRows.forEach((item) => {
        for (let i = state.approverList.length - 1; i >= 0; i--) {
          if (
            item.code == state.approverList[i].bizId &&
            state.approverList[i].subValue == state.orgId
          ) {
  ...
  const deleteApprover = (record: any) => {
    state.approverList.splice(
      state.approverList.findIndex(
        (item) => item.bizId === record.bizId && item.subValue == record.subValue,
      ),
      1,
    );
  };
