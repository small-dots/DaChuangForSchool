  ...
  >
    <div class="box">
      ...
      <div class="box-tab">
        <a-tabs v-model:activeKey="state.activeKey" @change="tabChange">
          <a-tab-pane
            :key="tabItem.key"
            :tab="tabItem.name"
            v-for="tabItem in state.tabList"
          ></a-tab-pane>
        </a-tabs>
      </div>
      <div class="box-content">
        ...
        <!-- 角色选择 -->
        <role-select
          v-show="state.activeKey == 'role'"
          ref="roleRef"
          @selectedChange="selectedChange"
        ></role-select>
        <!-- 职位选择 -->
        <position-select
          v-show="state.activeKey == 'position'"
          ref="positionRef"
          @selectedChange="selectedChange"
        ></position-select>
        <!-- 关系选择 -->
        <relation-select
          v-show="state.activeKey == 'relation'"
          ref="relationRef"
          @selectedChange="selectedChange"
        ></relation-select>
      </div>
...
<script setup lang="ts">
  ...
  const setData = (data: any) => {
    ...
    approverRef.value.state.approverList = data.selectOrgApproverTypeList
      ? data.selectOrgApproverTypeList
      : [];
  ...
  const selectedChange = () => {
    ...
    state.userGroupDetailName =
      userName + deptName + approverName + roleName + positionName + relationName;
  };
