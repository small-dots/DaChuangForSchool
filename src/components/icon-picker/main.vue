        ...
        <div @click.stop="">
          ...
          <div class="ele-icon-picker-pane">
            <a-menu v-model:selected-keys="selectedKeys" mode="inline">
              <a-menu-item v-for="(d, i) in activeList" :key="i">{{ d.title }} </a-menu-item>
            </a-menu>
            <div class="ele-icon-picker-list">
              <div class="ele-icon-picker-item" v-for="(d, i) in currentList" :key="i">
                <a-card hoverable>
                  <div class="ele-icon-picker-item-icon" :title="d" @click="onChoose(d)">
                    <component :is="d" />
                  </div>
                </a-card>
              </div>
      ...
      onSearch() {
        ...
        this.data.forEach((item, index) => {
          ...
          item.children.forEach((child) => {
            let icons = child.icons.filter(
              (d) => d.toLowerCase().indexOf(this.keywords.toLowerCase()) !== -1,
            );
            if (icons.length) {
              haveIcon = true;
